...
func (cfg Config) Clone() Config {
	c := cfg
	c.Multi, c.RequireAuth, c.set = slices.Clone(cfg.Multi), slices.Clone(cfg.RequireAuth), slices.Clone(cfg.set)
	c.Runtime = make(map[string]string, len(cfg.Runtime))
	for k, v := range cfg.Runtime { c.Runtime[k] = v }
	return c
}
...
func (cfg Config) tomap() map[string]string {
	...
	for k, v := range cfg.Runtime { o[k] = v }
	return o
}
